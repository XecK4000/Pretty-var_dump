...
function debug_dump() {
	...
	<div
		...
		style="border: 1px solid black;cursor: pointer;transition: background-color 100ms;">
...
function debug_recursive(&$key, &$element) {
	...
	switch($type) {
		...
		default:
			...
			echo '<div>'.($key !== NULL?'<span style="font-weight: bold;">'.$key.'</span><span style="color: grey;padding: 0px 10px;">➔</span>':'').'<span title="'.$title.'" onclick="navigator.clipboard.writeText(this.textContent);event.stopPropagation();" style="cursor: copy;padding: 1px 4px;background-color: '.$color[$type].';">'.$e.'</span></div>';
			break;
	}
}
